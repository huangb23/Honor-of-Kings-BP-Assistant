...
```bash
cd backend
../win_rate/.venv/bin/python -u main.py \
    --my 百里守约,墨子,元流之子(坦克) \
...
**未满 5v5 处理**：未满的阵容用"中性"补齐（缺位英雄胜率按 50、协同/克制按 0），这样部分阵容也能算出可比、不虚高的胜率。满 5 人后即为校准的绝对胜率。
...
- 保存到 `data/hero_winrate.json`（含全量英雄，胜率缺失的英雄用 50 兜底）。
